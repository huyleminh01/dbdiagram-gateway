...
const createStrategy = (strategy, proxyOptions, endpointUrls, req) => {
  const Strategy = strategies[strategy];
  return new Strategy(proxyOptions, endpointUrls, req);
};
...
module.exports = {
  ...
  policy: (params, config) => {
    ...
    return async function proxyHandler(req, res) {
      ...
      if (proxyOptions.stickyWSSession) {
        const { path } = req;
        const sessionId = path.replace('/', '');

        try {
          const address = await RedisClient.client.get(sessionId);
          Logger.debug('WS', sessionId, address);
          if (address) {
            target = parse(`ws://${address}:5001`);
          }
        } catch (error) {
          Logger.error(error);
        }
      ...
      if (req.headers.upgrade && req.headers.upgrade === 'websocket') {
        const wsProxy = httpProxy.createProxyServer({ target: `${target.protocol}//${target.host}` });
